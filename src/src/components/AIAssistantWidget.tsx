...
import { Send, X, User, RefreshCw, ShoppingBag, HelpCircle, MessageSquare } from "lucide-react";
...
export const AIAssistantWidget: React.FC<AIAssistantWidgetProps> = ({
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "welcome-1",
      sender: "bot",
      text: "Miawoezon ! 🇹🇬 Je suis **Aya**, l'Assistante IA d'Asime Togo. Comment puis-je vous aider aujourd'hui ?\n\n- Vous cherchez un **produit Made in Togo** (Miel, Karité, Café...) ?\n- Vous souhaitez connaître nos **frais et délais de livraison** ?\n- Vous souhaitez savoir **comment payer par T-Money ou Flooz** ?",
  ...
  return (
    <>
      ...
      <div className="fixed bottom-20 md:bottom-6 right-4 sm:right-6 z-50 flex flex-col items-end gap-2">
        {!isOpen && (
          <div className="bg-neutral-900 text-amber-300 text-[10px] font-bold px-2.5 py-1 rounded-full shadow-lg border border-amber-500/30 flex items-center gap-1.5 animate-bounce">
            <span>Besoin d'aide ? Parlons à Aya IA !</span>
          </div>
        )}

        <button
          onClick={() => setIsOpen(!isOpen)}
          className="w-13 h-13 sm:w-14 sm:h-14 rounded-full bg-neutral-950 text-[#d4af37] border-2 border-[#d4af37] shadow-2xl hover:scale-105 active:scale-95 transition-all cursor-pointer flex items-center justify-center relative group"
          title="Assistant IA Asime Togo"
          aria-label="Ouvrir l'assistant IA Aya"
        >
          {isOpen ? (
      ...
      {/* Chat Drawer Window */}
      {isOpen && (
        <div className="fixed bottom-24 md:bottom-22 right-3 sm:right-6 z-50 w-[92vw] sm:w-[400px] max-h-[80vh] h-[540px] bg-white border border-stone-200 rounded-2xl shadow-2xl flex flex-col overflow-hidden transition-all duration-300 font-sans">
          ...
          <div className="bg-neutral-950 text-white p-3.5 sm:p-4 flex items-center justify-between border-b border-amber-500/20">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-neutral-400">En ligne</p>
              </div>
            </div>
